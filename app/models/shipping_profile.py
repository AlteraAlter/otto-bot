...
class ShippingProfile(Base):
    ...
    order_cutoff: Mapped[str] = mapped_column(String, nullable=True)
    delivery_type: Mapped[str] = mapped_column(String, nullable=True)
    default_processing_time: Mapped[int] = mapped_column(Integer, nullable=True)
    transport_time: Mapped[int] = mapped_column(Integer, nullable=True)
